from app.schemas.order import (GetAllOrdersResponse, OrderConfirmResponse, OrderShippedRequest, OrderShippedResponse, OrderDeliveredRequest, OrderDeliveredResponse)
...
@router.get("/get_all_orders", response_model=GetAllOrdersResponse)
async def get_all_orders(user_id: Optional[int] = Depends(get_current_user), db: Session = Depends(get_db)):
    order_service = OrderService(db)
    return order_service.get_all(user_id)
